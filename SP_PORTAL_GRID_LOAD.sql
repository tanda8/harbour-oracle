...
BEGIN
	...
	WHERE RQBK_IDX.RQI_COMPLETE = -1   -- Completed Transaction
      ...
      --OR (i_SEARCH_TYPE = 'CN' AND DECODE(v_COMPANY_TYPE, 2, RQBK_SH_CS.RQSC_SH_NAME, 1, RQBK_SH_CS.RQSC_CS_NAME) = i_SEARCH_TERM)   -- Company Name
      OR (i_SEARCH_TYPE = 'CN' AND DECODE(v_COMPANY_TYPE, 2, LOWER(RQBK_SH_CS.RQSC_SH_NAME), 1, LOWER(RQBK_SH_CS.RQSC_CS_NAME)) = LOWER(i_SEARCH_TERM))   -- Company Name
			OR (i_SEARCH_TYPE = 'CBN' AND RQBK_OC_CARRIER.RQOC_BK_NUM = i_SEARCH_TERM)   -- Carrier Booking Number      
			OR (i_SEARCH_TYPE = 'SR' AND RQBK_REF.RQR_REF_TYPE = 1 AND RQBK_REF.RQR_REF_NUM = i_SEARCH_TERM)   -- Shipper Reference Number
			OR (i_SEARCH_TYPE = 'CR' AND RQBK_REF.RQR_REF_TYPE = 2 AND RQBK_REF.RQR_REF_NUM = i_SEARCH_TERM)   -- Consignee Reference Number
			OR (i_SEARCH_TYPE = 'AWN' AND RQBK_AIR_CARRIER.RQAC_AWB_NUM = i_SEARCH_TERM)   -- Air Waybill Number
      OR (i_SEARCH_TYPE = 'CT' AND RQBK_MC.RQM_COMM_NAME = i_SEARCH_TERM)   -- Commodity Type
      OR (i_SEARCH_TYPE = 'NBN' AND RQBK_OC_CARRIER.RQOC_NVO_NUM = i_SEARCH_TERM)   -- NVO Booking Number
      ...
      OR (i_SEARCH_TYPE = 'POD-Ci' AND CITY_POD.CT_DESC = i_SEARCH_TERM)   -- Place of Deleivery [City]
      OR (i_SEARCH_TYPE = 'POD-A' AND AIRPORTS_POD.AP_DESC = i_SEARCH_TERM)   -- Place of Deleivery [Airport]
      OR (i_SEARCH_TYPE = 'POD-AC' AND AIRPORTS_POD.AP_CODE = i_SEARCH_TERM)   -- Place of Deleivery [Airport Code]
      OR (i_SEARCH_TYPE = 'POD-Co' AND COUNTRY_POD.CY_DESC = i_SEARCH_TERM)   -- Place of Deleivery [Country]
		)
